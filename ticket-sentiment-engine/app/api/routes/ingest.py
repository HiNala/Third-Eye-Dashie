...
from app.models.ticket import RawTicket
...
async def ingest_tickets(
    ...
    Tickets are persisted to raw_tickets immediately and LLM processing is
    ...
    for i, item in enumerate(body.tickets):
        ...
        db.add(raw_ticket)
        await db.flush()  # populate raw_ticket.id
        ticket_ids.append(raw_ticket.id)
        logger.info(
            "  [%d/%d] Persisted raw ticket %s: title=%r, email=%s",
            i + 1,
            len(body.tickets),
            raw_ticket.id,
            item.title[:60],
    ...
    await db.commit()
    logger.info("All %d raw ticket(s) committed to DB", len(ticket_ids))
